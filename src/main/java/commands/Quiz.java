    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        ...
        try{
            ...
            if(message[0].equalsIgnoreCase("!quiz")){
                if(message.length>1) {
                    if(message[1].equalsIgnoreCase("help")){
                        event.getChannel().sendMessage("!quiz [course name] will return a random quiz of the course").queue();
                        event.getChannel().sendMessage("!quiz, !quiz java, !quiz database, !quiz javascript, !quiz php, ...").queue();
                        event.getChannel().sendMessage("How to answer? !@[your answer], " +
                                "How to store your quiz? !quizset:[course]:[question]:[answer], "+"Check your point by !quiz point, "+"Check top 10 by !quiz rank").queue();
                    }
            ...
            if(quizSet[0].equalsIgnoreCase("!quizset")){
                if(message.length<2){
                    event.getChannel().sendMessage("!quizset:[course]:[question]:[answer]").queue();
                }
                else{
                    try {
